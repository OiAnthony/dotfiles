...
- Default to the lowest detail level that fully answers the request. Simple facts and confirmations usually need one to three sentences.
- Expand only when requested or when detail changes understanding, implementation, or risk.
...
- Use one term per concept and make actors, conditions, actions, and results explicit when ambiguity is possible.
